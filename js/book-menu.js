...
function clear_invalid_syntax() {
  document
    .querySelectorAll(
      ".book-menu > :not(ul):not(h1):not(h2):not(h3):not(h4):not(h5):not(h6)"
    )
    .forEach((e) => {
...
function pack_menu_accordion() {
  const ulList = document.querySelectorAll(".book-menu > ul");
  console.log({ ulList });
  ulList.forEach((e, idx) => {
...
function highlight_current_tab() {
  const aTagList = document.querySelectorAll(".book-menu a");
